...
class ExpertPrior(OffPolicyAgent):
    def __init__(
            self,
            state_shape,
            action_dim,
    ...
    def train(self, states, actions, next_states, rewards, dones, weights=None):
        ...
        self._train_body(states, actions, next_states, rewards, dones, weights)
        ...
        if self.auto_alpha:
            tf.summary.scalar(name=self.policy_name + "/epsilon-KL", data=self.epsilon - kl_mean)
    ...
    def _train_body(self, states, actions, next_states, rewards, dones, weights):
        with tf.device(self.device):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                if self.auto_alpha:
                    policy_loss = tf.reduce_mean(self.alpha * (kl - self.epsilon) - current_min_q)
                else:
                    policy_loss = tf.reduce_mean(self.alpha * kl - current_min_q)

                # Compute loss of temperature parameter for KL  
                if self.auto_alpha:
                    alpha_loss = tf.reduce_mean(self.alpha * tf.stop_gradient(self.epsilon - kl))
